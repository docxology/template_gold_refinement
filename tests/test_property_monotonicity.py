...
class TestCanonicalPipelineProperty:
    def test_canonical_pipeline_is_monotone(self):
        ...
        for i in range(1, len(seq)):
            assert seq[i] > seq[i - 1], f"Stage {i}: {seq[i]} <= {seq[i - 1]}"
...
class TestKaratGradeProperty:
    ...
    @given(st.floats(min_value=0.375, max_value=1.0, allow_nan=False, allow_infinity=False))
    @settings(max_examples=100)
    def test_high_purity_has_karat(self, purity):
        grade = karat_for_purity(purity)
        assert grade.karat >= 9, f"Purity {purity} should be at least 9K"

    @given(st.floats(min_value=0.999, max_value=1.0, allow_nan=False, allow_infinity=False))
    @settings(max_examples=50)
...
class TestNinesProperty:
    @given(st.floats(min_value=0.9, max_value=1.0, allow_nan=False, allow_infinity=False))
    @settings(max_examples=100)
